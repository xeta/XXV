...
build_other: $(SRC_DIR)/other/MouseHunter.cpp $(SRC_DIR)/other/determinant.cpp $(SRC_DIR)/other/HanoiTower.cpp 
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -c $^
	
build_nlp: $(SRC_DIR)/nlp/distance.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -c $^
	
build_tests: build_all test_helper sorting_tests mouse_hunter_test tower_of_hanoi_test nlp_test
